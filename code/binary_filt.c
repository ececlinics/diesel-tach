...
uint8_t dyn_window_filt(uint8_t input){
	...
	static uint8_t next_data_index = 0; 
	static uint8_t next_pulse_index = 0;
	static uint8_t pulse_sum = 0;
	...
	// sum dynamic window
	if(next_data_index >= pulse_width-1)
	{//dynamic window does not wrap around
		for(i = next_data_index - pulse_width + 1;
			i < next_data_index + 1; i++)
		{//sums the dynamic window
			dyn_sum += data_history[i];
		}
	}
	else if(pulse_width < MAX_DATA_HISTORY)
	{//dynamic window does wrap around
		for(i = MAX_DATA_HISTORY + 1 + next_data_index-pulse_width;
			i != next_data_index + 1; i++)
		{//sums the dynamic window
